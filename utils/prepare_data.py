...
def demo():
    ...
    dataset = Kitti_2015_dataset(os.path.join(ROOT_DIR, 'data/KITTI/object'))
    data_idx = 0

    # Load data from data
    objects = dataset.get_label_objects(data_idx)
    objects[0].print_object()
    img = dataset.get_image_2(data_idx)
...
def extract_frustum_data(idx_filename, split, output_filename, viz=False,
    ...
    '''
    dataset = Kitti_2015_dataset(os.path.join(ROOT_DIR, 'data/KITTI/object'), split)
    ...
    for data_idx in data_idx_list:
        ...
        for obj_idx in range(len(objects)):
            ...
            # 2D BOX: Get pts0 rect backprojected
            ...
            for _ in range(augmentX):
                ...
                # 3D BOX: Get pts0 velo in 3d box
                obj = objects[obj_idx]
...
def write_2d_rgb_detection(det_filename, split, result_dir):
    ''' Write 2D detection outputs for KITTI evaluation.
    ...
    Input:
        det_filename: string, each line is
            img_path typeid confidence xmin ymin xmax ymax
        split: string, either trianing or testing
        result_dir: string, folder path for outputs dumping
    Output:
        None (will write <xxx>.txt files to disk)

    Usage:
        write_2d_rgb_detection("val_det.txt", "training", "outputs")
    '''
    dataset = Kitti_2015_dataset(os.path.join(ROOT_DIR, 'data/KITTI/object'), split)
    det_id_list, det_type_list, det_box2d_list, det_prob_list = \
        read_det_file(det_filename)
...
if __name__=='__main__':
    ...
    if args.gen_train:
        extract_frustum_data(\
            os.path.join(BASE_DIR, 'ImageSets/train.txt'),
            'training',
    ...
    if args.gen_val:
        extract_frustum_data(\
            os.path.join(BASE_DIR, 'ImageSets/val.txt'),
            'training',
