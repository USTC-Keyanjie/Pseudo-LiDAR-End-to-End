...
def load_part_ckpt(model, ckpt_file, logger=None, total_keys=-1):
    if os.path.isfile(ckpt_file):
        # print("==> Loading part model from checkpoint '{}'".format(ckpt_file))
        logger.info("==> Loading part model from checkpoint '{}'".format(ckpt_file))
        checkpoint = torch.load(ckpt_file)
        model_state = checkpoint['model_state']
        ...
        if update_keys == 0:
            raise RuntimeError
        logger.info("==> Done (loaded %d/%d)" % (update_keys, total_keys))
        # print("==> success! (loaded %d/%d)" % (update_keys, total_keys))
    else:
        print("==> failed! {} can not be loaded.".format(ckpt_file))
        ...
        with tqdm.trange(start_epoch, n_epochs, desc='epochs') as tbar, \
            ...
            for epoch in tbar:
                ...
                if trained_epoch % ckpt_save_interval == 0:
                    ckpt_name = os.path.join(self.ckpt_dir, 'checkpoint_epoch_%03d' % trained_epoch)
                    save_checkpoint(
