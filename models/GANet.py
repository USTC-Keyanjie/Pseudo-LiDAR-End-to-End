...
#     # Unpool: 3*3 unpooling with zero padding
#     def __init__(self, num_channels, stride=3):
...
class GANet(nn.Module):
    ...
    # def forward(self, x, y, f_bls):
    def forward(self, data):
        left = data['left']
        right = data['right']

        g = self.conv_start(left)
        left = self.feature(left)

        # ---------------- #
        # cspn_guidance = self.gud_up_proj_layer(x)
        # ---------------- #

        rem = left
        left = self.conv_x(left)

        right = self.feature(right)
        right = self.conv_y(right)

        left = self.cv(left, right)
        ...
        if self.training:
            disp0, disp1, disp2 = self.cost_agg(left, g)
            data['disp0'] = disp0
            data['disp1'] = disp1
            data['disp2'] = disp2
            return data
        else:
            disp2 = self.cost_agg(left, g)
            data['disp2'] = disp2
            return data
