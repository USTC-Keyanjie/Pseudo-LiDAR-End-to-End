...
from libs.GANet.modules import MyLoss2
...
def model_joint_fn_decorator():
    ...
    def model_fn(model, data_dict):
        # 输入数据整理部分，但是我在dataloader里整理好了
        # if cfg.RPN.ENABLED:
        ...
        if cfg.GANET.ENABLED and not cfg.GANET.FIXED:
            disp0 = ret_dict['disp0']
            disp1 = ret_dict['disp1']
            disp2 = ret_dict['disp2']

            ganet_target = input_data['ganet_target']
            ganet_target = torch.squeeze(ganet_target, 1)
            mask = ganet_target > 0
            mask.detach_()

            criterion = MyLoss2(thresh=3, alpha=2)
            ganet_loss = 0.2 * F.smooth_l1_loss(disp0[mask], ganet_target[mask], reduction='mean') + \
                         0.6 * F.smooth_l1_loss(disp1[mask], ganet_target[mask], reduction='mean') + \
                         criterion(disp2[mask], ganet_target[mask])

            loss += ganet_loss
            disp_dict['ganet_loss'] = ganet_loss.item()
            tb_dict['ganet_loss'] = ganet_loss.item()
    ...
    def get_rcnn_loss(model, ret_dict, tb_dict):
        ...
        if isinstance(model, nn.DataParallel):
            cls_loss_func = model.module.PointRCNN.rcnn_net.cls_loss_func
        else:
            cls_loss_func = model.PointRCNN.rcnn_net.cls_loss_func
