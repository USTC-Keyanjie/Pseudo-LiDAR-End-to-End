...
from libs.pointrcnn import kitti_utils
from models.PointRCNN.config import cfg
import libs.pointrcnn.roipool3d.roipool3d_utils as roipool3d_utils


class Kitti_VPNet_Dataset(Kitti_2015_Dataset):
    def __init__(self,
                 root_dir,
                 list_root,
                 mode='TRAIN',
        ...
        super().__init__(root_dir, list_root, mode)
        ...
        # PointRCNN config set
        if classes == 'Car':
            self.classes = ('Background', 'Car')
            aug_scene_root_dir = os.path.join(root_dir, 'KITTI', 'object', 'training', 'aug_scene')
        elif classes == 'People':
            self.classes = ('Background', 'Pedestrian', 'Cyclist')
        elif classes == 'Pedestrian':
            self.classes = ('Background', 'Pedestrian')
            aug_scene_root_dir = os.path.join(root_dir, 'KITTI', 'object', 'training', 'aug_scene_ped')
        elif classes == 'Cyclist':
            self.classes = ('Background', 'Cyclist')
            aug_scene_root_dir = os.path.join(root_dir, 'KITTI', 'object', 'training', 'aug_scene_cyclist')
        else:
        ...
        if self.mode == 'train_aug':
            self.aug_label_dir = os.path.join(aug_scene_root_dir, 'aug_label')
            self.aug_pts_dir = os.path.join(aug_scene_root_dir, 'rectified_data')
        else:
            self.aug_label_dir = os.path.join(aug_scene_root_dir, 'aug_label')
            self.aug_pts_dir = os.path.join(aug_scene_root_dir, 'rectified_data')
        ...
        if cfg.RPN.ENABLED or cfg.GANET.ENABLED:
            ...
            if mode == 'TRAIN':
                ...
                self.logger.info('Load testing samples from %s' % self.root_dir)
            ...
            print('Done: filter %s outputs for rcnn training: %d / %d\n' %
    ...
    def preprocess_rpn_training_data(self):
        ...
        如果一个图里面没有'车'，那么就去除这个图，不训练他
        最后留下的只有有'车'的图，数量为3265 / 3712
        """
        ...
        self.logger.info('Done: filter %s outputs: %d / %d\n' % (self.mode, len(self.sample_id_list),
    ...
    def get_image_2(self, idx):
        return super().get_image_2(idx % 10000)

    def get_image_3(self, idx):
        return super().get_image_3(idx % 10000)
    ...
    def get_rpn_sample(self, index):
        ...
        if sample_id < 10000:
            ...
            # pts_lidar = self.get_pseudo_lidar(sample_id)
    ...
    def __getitem__(self, index):
        if cfg.GANET.ENABLED:
            return self.get_ganet_sample(index)
        elif cfg.RPN.ENABLED:
    ...
    def get_ganet_sample(self, index):
        ...
        height = size[0]
        width = size[1]
        temp_data = np.zeros([7, height, width], 'float32')
        ...
        sample_info = {'left': left,
                       ...
                       'C2V': C2V,
                       'gt_boxes3d': gt_boxes3d,
                       'ganet_target': ganet_target,
                       }

        return sample_info

    def image_transform(self, temp_data, gt_boxes2d):
        ...
        left = temp_data[0: 3, :, :]
        right = temp_data[3: 6, :, :]
        ganet_target = temp_data[6, :, :]

        return left, right, ganet_target,\
               np.array([start_x], dtype='float32'), np.array([start_y], dtype='float32')
