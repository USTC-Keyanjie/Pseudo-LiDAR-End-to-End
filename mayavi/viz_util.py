...
def draw_lidar(pc, color=None, fig=None, bgcolor=(0, 0, 0), pts_scale=1, pts_mode='point', pts_color=None,
    ...
    '''
    if fig is None:
        fig = mlab.figure(figure=save_name, bgcolor=bgcolor, fgcolor=None, engine=None, size=(1600, 1000))
    if color is None:
        color = np.sqrt(pc[:, 0] ** 2 + pc[:, 1] ** 2)
...
if __name__ == '__main__':
    ...
    # calib_filepath = os.path.join(r"D:\DATASET\KITTI\object\data_object_calib\training\calib", "%06d.txt" % index)
    # calib = Calibration(calib_filepath=calib_filepath)
    # pc = calib.project_rect_to_velo(pc)
    fig = draw_lidar(pc)
