...
class Calibration(object):
    def __init__(self, calib_file):
        ...
        # Camera2 intrinsics and extrinsics
        self.cu = self.P2[0, 2]
        self.cv = self.P2[1, 2]
        self.fu = self.P2[0, 0]
        self.fv = self.P2[1, 1]
        self.bx = self.P2[0, 3] / (-self.fu)
        self.by = self.P2[1, 3] / (-self.fv)

        # Camera3 intrinsics and extrinsics
        self.fu3 = self.P3[0, 0]
        self.fv3 = self.P3[1, 1]
        self.bx3 = self.P3[0, 3] / (-self.fu3)
        self.by3 = self.P3[1, 3] / (-self.fv3)

        # other intrinsics
        self.f_bls = np.array([self.fu * (self.bx3 - self.bx)], dtype='float32')
    ...
    def img_to_rect(self, u, v, depth_rect):
        ...
        :return:
        """
        x = ((u - self.cu) * depth_rect) / self.fu + self.bx
        y = ((v - self.cv) * depth_rect) / self.fv + self.by
    ...
    def camera_dis_to_rect(self, u, v, d):
        ...
        x = ((u - self.cu) * d) / fd + self.bx
        y = ((v - self.cv) * d) / fd + self.by
